...
RSpec.describe "Visitor API flow", :smoke do
  ...
  let(:token) do
    visitor_response['Access-Token'].tap { |t| expect(t).not_to be_nil, "Expected Access-Token header" }
  end
  ...
  describe "scenario creation" do
    let(:scenario_response) do
      smoke_v1_post(
        ...
        token: token
      ).tap { |r| expect(r.code.to_i).to eq(201), "Expected 201 creating scenario, got #{r.code}: #{r.body}" }
    end
